...
import {
  ...
  getFileUrl,
  deleteFile,
  // Funções específicas de Pós-Consulta
  createPosConsultaFollowUpSequence,
  getPosConsultaFollowUpSequence,
  updatePosConsultaFollowUpSequence,

  createPosConsultaScheduleConfig,
  getPosConsultaScheduleConfig,
  updatePosConsultaScheduleConfig,
  deletePosConsultaScheduleConfig,
} from '../services/api';
...
// ----------------------------------------------------------------
// API Functions são importadas do api.ts
// ----------------------------------------------------------------
          ...
          <div key={step.step_number} className="relative flex items-center mb-12">
            ...
            <div className="ml-12 flex-1">
              <div className="text-sm text-slate-500 mb-1">
                Após {step.send_after} {unidadeTexto} da consulta
              </div>
...
// -------------------------------------------------------------
// Componente principal de PosConsultaConfig
// -------------------------------------------------------------
const PosConsultaConfig: React.FC = () => {
  ...
  // -----------------------------------------------------------
  // Carrega config de Pós-Consulta
  // -----------------------------------------------------------
  const carregarConfigExistente = async (companyId: number) => {
    try {
      setCarregando(true);
      const seq = await getPosConsultaFollowUpSequence(companyId);
  ...
  const handleLoadScheduleConfig = async (companyId: number) => {
    try {
      const resp = await getPosConsultaScheduleConfig(companyId);
      if (resp) {
  ...
  const handleSaveScheduleConfig = async () => {
    try {
      ...
      if (!scheduleExists) {
        await createPosConsultaScheduleConfig(companyIdNum, schedulePayload);
        setScheduleExists(true);
      } else {
        await updatePosConsultaScheduleConfig(companyIdNum, schedulePayload);
      }
  ...
  const handleDeleteScheduleConfig = async () => {
    try {
      ...
      await deletePosConsultaScheduleConfig(companyIdNum);
      setScheduleExists(false);
      setScheduleEditMode(false);
  ...
  const salvarConfig = async () => {
    try {
      ...
      const payload = {
        ...
        name: 'Sequência Pós-Consulta',
        description: 'Mensagens automáticas para leads após consulta visando conversão',
        steps: passosAPI,
      };

      // create ou update
      if (hasSequence) {
        await updatePosConsultaFollowUpSequence(companyIdNum, payload);
        setSavedStatus('saved');
      } else {
        await createPosConsultaFollowUpSequence(companyIdNum, payload);
        setHasSequence(true);
        setSavedStatus('saved');
      }
    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Follow-up Pós-Consulta</h1>
            <p className="text-slate-600 mt-2">Configure mensagens automáticas para converter leads após consultas</p>
          </div>
                  ...
                  {passos.length > 0 ? (
                    ...
                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-lg p-8 text-center">
                      ...
                      <p className="text-slate-500 mb-4">
                        Nenhum passo configurado. Crie sua sequência de follow-up pós-consulta.
                      </p>
                      <button
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">
                                Após {passo.send_after} {unidadeTempoTexto(passo.send_after_unit)} da consulta
...
export default PosConsultaConfig;