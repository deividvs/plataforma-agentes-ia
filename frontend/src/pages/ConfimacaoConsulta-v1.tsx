...
import {
  // Funções de CONFIRMATION
  createConfirmationSequence,
  getConfirmationSequence,
  updateConfirmationSequence,
  deleteConfirmationSequence,

  // Funções de CONFIRMATION SCHEDULE
  getConfirmationScheduleConfig,
  createConfirmationScheduleConfig,
  updateConfirmationScheduleConfig,
  deleteConfirmationScheduleConfig,

  // Funções de upload e delete de arquivos
  uploadFile,
  deleteFile,
  getFileUrl,
} from '../services/api';
...
// ------------------------------------------------------------------
// 1) Tipos LOCAIS (React state) agora com "id?" para Steps/Mensagens
// ------------------------------------------------------------------
interface MensagemLocal {
  id?: number;  // <--- agora podemos ter ID
  type: 'text' | 'image' | 'audio' | 'video';
  content: string | File;
}

interface PassoLocal {
  id?: number;  // <--- idem, pode ter ID
  step_number: number;
  send_before: number;                         // front -> "antes"
  send_before_unit: 'days' | 'hours' | 'minutes';
  messages: MensagemLocal[];
}

// ------------------------------------------------------------------
// 2) Tipos para API (com "id?" também), mas "content" sempre string
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
// Tipos para Schedule, inalterados
// ------------------------------------------------------------------
...
const TimelineIndicator = ({ passos, onEditStep, onAddStep }: {
  ...
  return (
    <div className="relative mt-8 mb-6">
      ...
      {passos.map((step) => {
        const unidadeTexto = step.send_before_unit === 'days' ? 'dias' :
                        step.send_before_unit === 'hours' ? 'horas' : 'minutos';
          ...
          <div key={step.step_number} className="relative flex items-center mb-12">
            ...
            <div className="ml-12 flex-1">
              <div className="text-sm text-slate-500 mb-1">
                {step.send_before} {unidadeTexto} antes da consulta
              </div>
...
const Confirmation: React.FC = () => {
  ...
  // -----------------------------------------------------------
  // Carrega config de Confirmação
  // -----------------------------------------------------------
  const carregarConfigExistente = async (companyId: number) => {
    try {
      setCarregando(true);
      const seq = await getConfirmationSequence(companyId);
      ...
      if (seq === null) {
        ...
        const passosConvertidos: PassoLocal[] = seq.steps.map((step: any) => ({
          ...
          send_before: step.send_after,
          send_before_unit: step.send_after_unit as 'days' | 'hours' | 'minutes',
  ...
  // -----------------------------------------------------------
// Função corrigida para carregar configuração de horários
const handleLoadScheduleConfig = async (companyId: number) => {
  try {
    const resp = await getConfirmationScheduleConfig(companyId);
    if (resp) {
  ...
  const handleSaveScheduleConfig = async () => {
    try {
      ...
      if (!scheduleExists) {
        await createConfirmationScheduleConfig(companyIdNum, schedulePayload);
        setScheduleExists(true);
      } else {
        await updateConfirmationScheduleConfig(companyIdNum, schedulePayload);
      }
  ...
  const handleDeleteScheduleConfig = async () => {
    try {
      ...
      await deleteConfirmationScheduleConfig(companyIdNum);
      setScheduleExists(false);
      setScheduleEditMode(false);
  ...
  const salvarConfig = async () => {
    try {
      ...
      if (!companyIdStr || !clientIdStr) {
        throw new Error('ID da empresa ou cliente não encontrado');
      }

      const companyIdNum = Number(companyIdStr);

      // 1) Transformar PassoLocal[] em PassoAPI[] (fazendo upload se houver File)
      const passosAPI: PassoAPI[] = await Promise.all(
        passos.map(async (passoLocal) => {
          // Ordenamos as mensagens pelo ID
          ...
          return {
            ...
            send_after: passoLocal.send_before,
            send_after_unit: passoLocal.send_before_unit,
      ...
      const payload = {
        ...
        name: 'Sequência de Confirmação',
        description: 'Mensagens automáticas de confirmação',
        steps: passosAPI,
      };

      // 3) create ou update
      if (hasSequence) {
        await updateConfirmationSequence(companyIdNum, payload);
        setSavedStatus('saved');
      } else {
        await createConfirmationSequence(companyIdNum, payload);
        setHasSequence(true);
        setSavedStatus('saved');
      }
  ...
  const handleInsertPlaceholder = (
    ...
    const passoIndex = passos.findIndex((p) => p.step_number === stepNumber);
    if (passoIndex === -1) return;

    const passo = passos[passoIndex];
    const mensagem = passo.messages[messageIndex];

    if (mensagem.type === 'text' && typeof mensagem.content === 'string') {
      const updatedContent = mensagem.content + ' ' + placeholder;

      alterarMensagem(stepNumber, messageIndex, 'text', updatedContent);
    }
  };
    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-slate-800">Sequência de Confirmação</h1>
                  ...
                  {passos.length > 0 ? (
                    ...
                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-lg p-8 text-center">
                      ...
                      <p className="text-slate-500 mb-4">
                        Nenhum passo configurado. Crie sua sequência de confirmação.
                      </p>
                      <button
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">
                                {passo.send_before} {unidadeTempoTexto(passo.send_before_unit)} antes da consulta
                    ...
                    <div className="flex gap-3">
                      <div className="w-24">
                        <input
                          type="number"
                          min="1"
                          value={editandoPasso.send_before}
                          onChange={(e) => atualizarPasso({
                            ...editandoPasso,
                            send_before: parseInt(e.target.value || '1', 10),
                          })}
                      ...
                      <div className="w-32">
                        <select
                          value={editandoPasso.send_before_unit}
                          onChange={(e) => atualizarPasso({
                            ...editandoPasso,
                            send_before_unit: e.target.value as 'days' | 'hours' | 'minutes',
                          })}
                          className="w-full rounded-md border-slate-300 shadow-sm focus:border-slate-500 focus:ring focus:ring-slate-200 focus:ring-opacity-50"
                        >
                          {timeUnits.map(unit => (
                    ...
                    {editandoPasso.messages.length > 0 ? (
                      editandoPasso.messages.map((message, index) => (
                        <div key={index} className="bg-slate-50 rounded-lg p-4 mb-4">
                          ...
                          {message.type === 'text' ? (
                            <div>
                              {/* Botões de placeholder */}
                              <div className="flex flex-wrap items-center gap-2 mb-2">
                                <button
                                  type="button"
                                  onClick={() => handleInsertPlaceholder(editandoPasso.step_number, index, '[data-consulta]')}
