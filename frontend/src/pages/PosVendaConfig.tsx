...
import {
  // Funções para upload e preview (reutilizadas)
  uploadFile,
  getFileUrl,
  deleteFile,
  // Funções específicas de Pós-Venda
  createPosVendaFollowUpSequence,
  getPosVendaFollowUpSequence,
  updatePosVendaFollowUpSequence,

  createPosVendaScheduleConfig,
  getPosVendaScheduleConfig,
  updatePosVendaScheduleConfig,
  deletePosVendaScheduleConfig,
} from '../services/api';
...
const messageTypes = [
  ...
  { value: 'nps', label: 'Pesquisa NPS', icon: Star }
];

// ----------------------------------------------------------------
// API Functions (temporárias até serem implementadas no api.ts)
// ----------------------------------------------------------------

// Funções temporárias simuladas para desenvolvimento
// COMENTADAS - Agora estamos usando as funções reais da API
/*
const createPosVendaFollowUpSequence = async (companyId: number, payload: any) => {
  console.log('createPosVendaFollowUpSequence not implemented yet', { companyId, payload });
  throw new Error('API não implementada ainda');
};

const getPosVendaFollowUpSequence = async (companyId: number) => {
  console.log('getPosVendaFollowUpSequence not implemented yet', { companyId });
  return null; // Simula que não há sequência
};

const updatePosVendaFollowUpSequence = async (companyId: number, payload: any) => {
  console.log('updatePosVendaFollowUpSequence not implemented yet', { companyId, payload });
  throw new Error('API não implementada ainda');
};

const createPosVendaScheduleConfig = async (companyId: number, scheduleData: any) => {
  console.log('createPosVendaScheduleConfig not implemented yet', { companyId, scheduleData });
  throw new Error('API não implementada ainda');
};

const getPosVendaScheduleConfig = async (companyId: number) => {
  console.log('getPosVendaScheduleConfig not implemented yet', { companyId });
  return null; // Simula que não há configuração
};

const updatePosVendaScheduleConfig = async (companyId: number, scheduleData: any) => {
  console.log('updatePosVendaScheduleConfig not implemented yet', { companyId, scheduleData });
  throw new Error('API não implementada ainda');
};

const deletePosVendaScheduleConfig = async (companyId: number) => {
  console.log('deletePosVendaScheduleConfig not implemented yet', { companyId });
  throw new Error('API não implementada ainda');
};
*/
...
const MessagePreview = ({ message }: { message: MensagemLocal }) => {
  ...
  if (type === 'nps' && typeof content === 'string') {
    return (
      <div className="bg-blue-50 border-l-4 border-blue-400 p-3 rounded-lg">
        <div className="flex items-center mb-2">
          <Star className="w-4 h-4 text-blue-600 mr-2" />
          <span className="text-sm font-medium text-blue-800">Pesquisa de Satisfação (NPS)</span>
        </div>
        <p className="text-slate-700 text-sm">{content}</p>
        <div className="mt-2 flex items-center space-x-1">
          {[1, 2, 3, 4, 5].map((star) => (
            <span key={star} className="text-xs bg-blue-100 px-2 py-1 rounded flex items-center">
              {star} ⭐
            </span>
          ))}
        </div>
          ...
          <div key={step.step_number} className="relative flex items-center mb-12">
            ...
            <div className="ml-12 flex-1">
              <div className="text-sm text-slate-500 mb-1">
                Após {step.send_after} {unidadeTexto} da venda
              </div>
...
// -------------------------------------------------------------
// Componente principal de PosVendaConfig
// -------------------------------------------------------------
const PosVendaConfig: React.FC = () => {
  ...
  // -----------------------------------------------------------
  // Carrega config de Pós-Venda
  // -----------------------------------------------------------
  const carregarConfigExistente = async (companyId: number) => {
    try {
      setCarregando(true);
      const seq = await getPosVendaFollowUpSequence(companyId);
      ...
      if (!seq) {
        ...
        const passosConvertidos: PassoLocal[] = seq.steps.map((step: any) => ({
          ...
          messages: step.messages.map((msg: any) => ({
            id: msg.id,
            type: msg.type as 'text' | 'image' | 'audio' | 'video' | 'nps',
            content: msg.content,
          })),
        }));
  ...
  const handleLoadScheduleConfig = async (companyId: number) => {
    try {
      const resp = await getPosVendaScheduleConfig(companyId);
      if (resp) {
  ...
  const handleSaveScheduleConfig = async () => {
    try {
      ...
      if (!scheduleExists) {
        await createPosVendaScheduleConfig(companyIdNum, schedulePayload);
        setScheduleExists(true);
      } else {
        await updatePosVendaScheduleConfig(companyIdNum, schedulePayload);
      }
  ...
  const handleDeleteScheduleConfig = async () => {
    try {
      ...
      await deletePosVendaScheduleConfig(companyIdNum);
      setScheduleExists(false);
      setScheduleEditMode(false);
  ...
  const salvarConfig = async () => {
    try {
      ...
      // Transformar PassoLocal[] em API[] (fazendo upload se houver File)
        ...
        passos.map(async (passoLocal) => {
          const messagesAPI = await Promise.all(
            passoLocal.messages.map(async (msgLocal) => {
              // Para NPS e texto, garantir que seja string
              if (msgLocal.type === 'nps' || msgLocal.type === 'text') {
                return {
                  id: msgLocal.id,
                  type: msgLocal.type,
                  content: typeof msgLocal.content === 'string' ? msgLocal.content : '',
                };
              }

              // Verificamos se é um arquivo para tipos de mídia
              if ((msgLocal.type === 'image' || msgLocal.type === 'audio' || msgLocal.type === 'video') && isFile(msgLocal.content)) {
      ...
      // Montamos o payload final
      const payload = {
        company_id: companyIdNum,
        name: 'Sequência Pós-Venda',
        description: 'Mensagens automáticas para clientes após venda visando fidelização e feedback',
        steps: passosAPI,
      };

      // create ou update
      if (hasSequence) {
        await updatePosVendaFollowUpSequence(companyIdNum, payload);
        setSavedStatus('saved');
      } else {
        await createPosVendaFollowUpSequence(companyIdNum, payload);
        setHasSequence(true);
        setSavedStatus('saved');
      }
      ...
      // Recarrega do GET para atualizar e ordenar local
  ...
  const deletarMensagem = async (numeroPasso: number, indiceMensagem: number) => {
    try {
      ...
      if (mensagem && mensagem.type !== 'text' && mensagem.type !== 'nps' && typeof mensagem.content === 'string') {
  ...
  const alterarMensagem = async (
    ...
    messageType: 'text' | 'image' | 'audio' | 'video' | 'nps',
    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Follow-up Pós-Venda</h1>
            <p className="text-slate-600 mt-2">Configure mensagens automáticas para fidelizar clientes após vendas</p>
          </div>
        ...
        {savedStatus && (
          <div className={`fixed bottom-6 right-6 py-2 px-4 rounded-md shadow-lg flex items-center gap-2 ${
            savedStatus === 'saving' ? 'bg-blue-100 text-slate-700' :
        ...
        {activeTab === 'sequence' && (
          <div className="mb-8">
            ...
            {!editandoPasso ? (
                  ...
                  {passos.length > 0 ? (
                    ...
                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-lg p-8 text-center">
                      ...
                      <p className="text-slate-500 mb-4">
                        Nenhum passo configurado. Crie sua sequência de follow-up pós-venda.
                      </p>
                      <button
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">
                                Após {passo.send_after} {unidadeTempoTexto(passo.send_after_unit)} da venda
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <span className="text-sm text-slate-500">
                                    Mensagem {msgIndex + 1} • {
                                      ...
                                      message.type === 'nps' ? 'Pesquisa NPS' : 'Mídia'
                                    }
              ...
              /* Formulário de edição de passo - mesmo código do PosConsultaConfig mas com textos adaptados */
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  {/* Messages - mesmo código do PosConsultaConfig */}
                  <div className="mb-6">
                    ...
                    {/* Mensagens existentes - código similar ao PosConsultaConfig */}
                      ...
                      editandoPasso.messages.map((message, index) => (
                        <div key={index} className="bg-slate-50 rounded-lg p-4 mb-4">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <select
                                ...
                                onChange={(e) => alterarMensagem(
                                  editandoPasso.step_number,
                                  index,
                                  e.target.value as 'text' | 'image' | 'audio' | 'video' | 'nps',
                          ...
                          {message.type === 'text' ? (
                            ...
                            <div>
                              <div className="w-full rounded-md border-slate-300 shadow-sm bg-slate-100 p-3 text-slate-600 cursor-not-allowed">
                                Em uma escala de 1 a 5, como você avalia nosso atendimento?
                              </div>
                              <div className="mt-2 p-3 bg-blue-50 rounded-md">
                                <div className="flex items-center mb-2">
                                  <Star className="w-4 h-4 text-blue-600 mr-2" />
                                  <span className="text-sm text-blue-800 font-medium">Pesquisa NPS (Padrão do Sistema)</span>
                                </div>
                                <p className="text-xs text-blue-600">
                                  Esta mensagem segue o padrão do sistema e não pode ser editada. Será enviada uma pesquisa de 1 a 5 estrelas para o cliente.
                                </p>
                                <div className="mt-2 flex items-center space-x-1">
                                  {[1, 2, 3, 4, 5].map((star) => (
                                    <span key={star} className="text-xs bg-blue-100 px-2 py-1 rounded flex items-center">
                            ...
                            /* File upload section - mesmo código do PosConsultaConfig */
                            <div className="mt-2">
                              {/* Arquivo já enviado */}
        ...
        {activeTab === 'schedule' && (
          <div className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div className="flex justify-end mt-6 gap-3">
                ...
                <>
                  {scheduleEditMode ? (
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (window.confirm('Tem certeza que deseja remover a configuração de horários?')) {
                          await handleDeleteScheduleConfig();
                        }
                      }}
                    >
              ...
              {/* Se não existir config ou estiver editando, mostra salvar */}
              {(!scheduleExists || scheduleEditMode) && (
                <button
                  onClick={handleSaveScheduleConfig}
