...
class AICreditPurchase(Base):
    """Pending/confirmed external purchase that can grant AI credits once."""
    ...
    provider = Column(String(30), nullable=False, server_default="eduzz")
    provider_product_id = Column(String(120), nullable=False)
    ...
    currency = Column(String(10), nullable=False, server_default="BRL")
    status = Column(String(30), nullable=False, server_default="pending")

    payment_url = Column(Text, nullable=True)
    eduzz_cart_id = Column(String(120), nullable=True)
    eduzz_cart_key = Column(String(255), nullable=True)
    eduzz_invoice_id = Column(String(120), nullable=True)
    eduzz_transaction_id = Column(String(120), nullable=True)
    eduzz_webhook_event_id = Column(String(120), nullable=True)
    credited_transaction_id = Column(BigInteger, ForeignKey("ai_credit_transactions.id", ondelete="SET NULL"), nullable=True)

    checkout_request = Column(JSONB, nullable=False, server_default="{}")
    checkout_response = Column(JSONB, nullable=False, server_default="{}")
    webhook_payload = Column(JSONB, nullable=False, server_default="{}")
    purchase_metadata = Column(JSONB, nullable=False, server_default="{}")
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now())
    updated_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now())
    credited_at = Column(TIMESTAMP(timezone=True), nullable=True)

    company = relationship("Company", foreign_keys=[company_id])
    credited_transaction = relationship("AICreditTransaction", foreign_keys=[credited_transaction_id])

    __table_args__ = (
        UniqueConstraint("order_id", name="uq_ai_credit_purchases_order_id"),
        UniqueConstraint("eduzz_invoice_id", name="uq_ai_credit_purchases_eduzz_invoice"),
        UniqueConstraint("eduzz_transaction_id", name="uq_ai_credit_purchases_eduzz_transaction"),
        UniqueConstraint("eduzz_webhook_event_id", name="uq_ai_credit_purchases_eduzz_event"),
        CheckConstraint(
            "status IN ('pending', 'checkout_created', 'credited', 'failed')",
            name="chk_ai_credit_purchase_status",
        ),
        Index("idx_ai_credit_purchases_company_created", "company_id", "created_at"),
