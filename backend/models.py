...
class RefundAccessSuspension(Base):
    """Durable account block keyed by the originating sale/invoice."""

    __tablename__ = "refund_access_suspensions"

    id = Column(BigInteger, primary_key=True, autoincrement=True)
    source = Column(String(40), nullable=False)
    invoice_id = Column(String(255), nullable=False)
    email_hash = Column(String(64), nullable=False)
    root_client_id = Column(Integer, ForeignKey("clients.id", ondelete="SET NULL"), nullable=True)
    state = Column(String(30), nullable=False, server_default="refund_pending")
    latest_event_id = Column(String(255), nullable=False)
    latest_occurred_at = Column(TIMESTAMP(timezone=True), nullable=False)
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)

    __table_args__ = (
        UniqueConstraint("source", "invoice_id", name="uq_refund_access_suspension_source_invoice"),
        CheckConstraint(
            "state IN ('refund_pending', 'refunded', 'active')",
            name="chk_refund_access_suspension_state",
        ),
        Index("idx_refund_access_suspensions_client_state", "root_client_id", "state"),
        Index("idx_refund_access_suspensions_email_hash", "email_hash"),
    )


class RefundAccessEvent(Base):
    """Idempotency/audit record for a refund access transition."""

    __tablename__ = "refund_access_events"
    ...
    suspension_id = Column(
        BigInteger,
        ForeignKey("refund_access_suspensions.id", ondelete="SET NULL"),
        nullable=True,
    )
    requested_state = Column(String(30), nullable=False)
    ...
    __table_args__ = (
        CheckConstraint(
            "requested_state IN ('refund_pending', 'refunded', 'active')",
            name="chk_refund_access_event_requested_state",
        ),
        CheckConstraint(
            "status IN ('processing', 'completed', 'failed')",
            name="chk_refund_access_event_status",
        ),
        Index("idx_refund_access_events_suspension", "suspension_id"),
        Index("idx_refund_access_events_status", "status"),
    )


class RefundAccessTarget(Base):
    """Snapshot used to restore only entities changed by one suspension."""

    __tablename__ = "refund_access_targets"

    id = Column(BigInteger, primary_key=True, autoincrement=True)
    suspension_id = Column(
        BigInteger,
        ForeignKey("refund_access_suspensions.id", ondelete="CASCADE"),
        nullable=False,
    )
    entity_type = Column(String(20), nullable=False)
    entity_id = Column(BigInteger, nullable=False)
    previous_active = Column(Boolean, nullable=True)
    previous_waha_enabled = Column(Boolean, nullable=True)
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "entity_type",
            "entity_id",
            name="uq_refund_access_target_entity",
        ),
        CheckConstraint(
            "entity_type IN ('company', 'client', 'user')",
            name="chk_refund_access_target_entity_type",
        ),
        Index("idx_refund_access_targets_entity", "entity_type", "entity_id"),
    )


class RefundAccessEntityState(Base):
    """Baseline retained for the whole overlapping refund-block episode."""

    __tablename__ = "refund_access_entity_states"

    id = Column(BigInteger, primary_key=True, autoincrement=True)
    entity_type = Column(String(20), nullable=False)
    entity_id = Column(BigInteger, nullable=False)
    restore_active = Column(Boolean, nullable=True)
    restore_waha_enabled = Column(Boolean, nullable=True)
    waha_was_finalized = Column(Boolean, nullable=False, server_default="false")
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)

    __table_args__ = (
        UniqueConstraint("entity_type", "entity_id", name="uq_refund_access_entity_state"),
        CheckConstraint(
            "entity_type IN ('company', 'client', 'user')",
            name="chk_refund_access_entity_state_type",
        ),
    )
